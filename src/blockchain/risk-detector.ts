  ...
  if (concentrationPct > CONCENTRATION_THRESHOLD_PCT) {
    ...
    findings.push(
      makeFinding(
        ...
        "FUNDAMENTAL",
        "power-concentration",
        `Top ${TOP_N_CONCENTRATION} wallets control ${concentrationPct.toFixed(1)}% of sampled power index`,
        `The ${TOP_N_CONCENTRATION} most powerful wallets collectively hold ${concentrationPct.toFixed(1)}% of the power index across the ${rankings.length} sampled wallets (not the entire network), exceeding the ${CONCENTRATION_THRESHOLD_PCT}% threshold. This indicates concentration risk within the observable sample.`,
        {
          ...
          note: "Denominator is the sum of the sampled wallets, not total network power.",
        },
